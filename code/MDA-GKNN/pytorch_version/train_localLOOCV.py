...
def evaluate_full_batch(model, minibatch, lloocv_idx, mode='val'):
    """
    ...
    for n in node_target:
        #print(labels.shape, preds.shape)
        #print('n = ', n)
        labels, preds = to_numpy(labels[:, 1]), to_numpy(preds[:, 1])
        candidate_idx = lloocv_idx[np.where(np.array(labels)[lloocv_idx] == 0)]
        n = np.hstack([n, candidate_idx])
        #print(labels, preds, candidate_idx, n)
        #print(labels[n].shape, preds[n].shape)

        ys, performances = globalloocv_metrics(labels[n], preds[n], model.sigmoid_loss) # localloocv一样的
...
def train(train_phases, model, minibatch, minibatch_eval, model_eval, eval_val_every, lloocv_idx):
...
style = 'yellow')
                ...
                loss_val, ys_val, metrics_val = evaluate_full_batch(model_eval, minibatch_eval, lloocv_idx, mode='val') #llobal_loocv
    ...
    loss_test, ys_lloocv, metrics_test = evaluate_full_batch(model_eval, minibatch_eval, lloocv_idx, mode='val')
...
if __name__ == '__main__':
    ...
    fold = eval(args_global.data_prefix.split('/')[-1].split('_')[-1][4:])
    train_test_id_idx = np.load('/home/chujunyi/4_GNN/GraphSAINT/miRNA_disease_data/global_LOOCV/task_Tp__testlabel0_knn_edge_train_test_index_all.npz', allow_pickle = True)
    train_index_all = train_test_id_idx['train_index_all']
    test_index_all = train_test_id_idx['test_index_all']
    train_id_all = train_test_id_idx['train_id_all']
    test_id_all = train_test_id_idx['test_id_all']
    lloocv_test_disease_id = test_id_all[fold][0, 1]
    lloocv_idx = np.where(train_id_all[fold][:, 1] == lloocv_test_disease_id)[0]
    lloocv_idx = train_index_all[fold][lloocv_idx]
    print('fold = {} | lloocv_idx = {}'.format(fold, lloocv_idx))

    
    model, minibatch, minibatch_eval, model_eval = prepare(train_data, train_params, arch_gcn)
    ys_lloocv = train(train_phases, model, minibatch, minibatch_eval, model_eval, train_params['eval_val_every'], lloocv_idx)
    np.savez_compressed('/home/chujunyi/4_GNN/GraphSAINT/miRNA_disease__localLOOCV/y_val/' + args_global.data_prefix.split('/')[-1], y_lloocv_true = ys_lloocv[0], y_lloocv_pred = ys_lloocv[1], y_lloocv_prob = ys_lloocv[2])
    #python -m graphsaint.pytorch_version.train_localLOOCV --data_prefix miRNA_disease_data/global_LOOCV/graphsaint_data/task_Tp__testlabel0_10knn_edge_fold0 --train_config ./train_config/table2/ppi2_e_lr0.001.yml --gpu 0