...
def evaluate_full_batch(model, minibatch, mode='val'):
    ...
    for n in node_target:
        ...
        candidate_idx = np.where(labels == 0)[0]
        n = np.hstack([n, candidate_idx])
        #print(labels, preds, candidate_idx, n)
        #print(labels[n].shape, preds[n].shape)

        ys, performances = globalloocv_metrics(labels[n], preds[n], model.sigmoid_loss)
...
style = 'yellow')
                ...
                loss_val, ys_val, metrics_val = evaluate_full_batch(model_eval, minibatch_eval, mode='val') #global_loocv
    ...
    loss_test, ys_gloocv, metrics_test = evaluate_full_batch(model_eval, minibatch_eval, mode='val')
...
if __name__ == '__main__':
    ...
    ys_gloocv = train(train_phases, model, minibatch, minibatch_eval, model_eval, train_params['eval_val_every'])
    np.savez_compressed('/home/chujunyi/4_GNN/GraphSAINT/miRNA_disease__globalLOOCV/y_val/' + args_global.data_prefix.split('/')[-1], y_gloocv_true = ys_gloocv[0], y_gloocv_pred = ys_gloocv[1], y_gloocv_prob = ys_gloocv[2])
    #python -m graphsaint.pytorch_version.train_globalLOOCV --data_prefix miRNA_disease_data/global_LOOCV/graphsaint_data/task_Tp__testlabel0_10knn_edge_fold0 --train_config ./train_config/table2/ppi2_e_lr0.001.yml --gpu 0